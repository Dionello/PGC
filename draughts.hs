...
--exchangePositions :: mudar 2 espaços da tabela...

-- executa uma jogada no tabuleiro
makeMove :: Move -> Board -> Board
makeMove (mv,cd,qd) bd | not (isValid (mv,cd,qd)) = error "Move is not valid"
                       | mv == Walk = insertAt Nothing cd (insertAt piece dest bd)
                       | otherwise  = (insertAt Nothing cd (insertAt piece dest bd))
                   where dest = destination Move
                         piece = spaceAt cd qd
                         exchange 
